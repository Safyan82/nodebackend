...
exports.fbInfo=async(req,res)=>{

    
    fb.getInfo(req.body.url).then((info) => {
    ...
    await https.get(req.body.downloadUrl, function(response) {
        ...
        response.pipe(file)
            ...
            response.on("data", function(chunk) {
                ...
                app.ioSocket.emit("message",{downloaded:(cur / 1048576).toFixed(2) + " mb",total:total.toFixed(2),percentage:(100.0 * cur / len).toFixed(2) + "% "})            
            ...
            response.on("end", function() {
                ...
                app.ioSocket.emit("message",{downloaded:total.toFixed(2)+ " mb",total:total.toFixed(2),percentage:"completed"});            
